...
<h3 class="page-title">
    Model Pelanggan &nbsp;&nbsp;
    <a href="{{route('modelpelanggans.create')}}" class="btn btn-primary btn-sm">
...
<tbody>
    ...
    <td>{{ $d->jenismodel->nama_jenismodel }}</td>
    <td>{{ $d->pelanggan->nama_pelanggan }}</td>
    <td>
        <ul class="nav nav-pills">
            <li >
                <button onclick="window.location='{{ route('modelpelanggans.edit', $d->id) }}'" type="button" class="btn btn-success">Ubah</button>
            </li>
            <li>
                <form method="POST" action="{{route('modelpelanggans.destroy' , $d->id)}}">
                    @method('DELETE')
                    @csrf
