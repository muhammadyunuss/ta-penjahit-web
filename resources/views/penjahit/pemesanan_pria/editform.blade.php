...
<div class="page-bar">
    <ul class="page-breadcrumb">
        ...
        <li>
            <a href="{{route('pemesananprias.update', $data->id)}}">Ubah Pemesanan Pria</a>
        </li>
    </ul>
</div>
<!-- END PAGE HEADER-->
...
<div class="portlet">
		<div class="portlet-title">
			<div class="caption">
				<i class="fa fa-reorder"></i> Ubah Pemesanan Pria
			</div>
		</div>
		<div class="portlet-body form">
			<form method="POST" action="{{ route('pemesananprias.update', $data->id) }}" enctype="multipart/form-data">
			@csrf
            @method("PUT")
				<div class="form-body">
                        ...
                        <div class="col-md-12">
                            <select name="nmPelanggan" id="nmPelanggan" class="form-control @error('nmPelanggan') is-invalid @enderror">
                                ...
                                @foreach($datapelanggan as $dp)
                                    <option value="{{ $dp->id }}" {{ old('nmPelanggan', $data->id) == $dp->id ? 'selected' : null }}>{{ $dp->nama_pelanggan }}</option>
                                @endforeach
                            </select>
                            @error('nmPelanggan')
                                <div class="invalid-feedback" style="color:red">{{ $message }}</div>
                        ...
                        <div class="col-md-12">
                            <select name="nmModel" id="nmModel" data-with="100%" class="form-control @error('nmModel') is-invalid @enderror">
                                ...
                                @foreach($datamodel as $dm)
                                    <option value="{{ $dm->id }}" {{ old('nmModel', $data->id) == $dm->id ? 'selected' : null }}>{{ $dm->nama_model }}</option>
                                @endforeach
                            </select>
                            @error('nmModel')
                                <div class="invalid-feedback" style="color:red">{{ $message }}</div>
